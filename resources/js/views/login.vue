        ...
        <div class="flex w-full lg:w-1/2 justify-center items-center bg-white space-y-8">
            <div class="w-full px-8 md:px-32 lg:px-24">
                <form class="bg-white rounded-md shadow-2xl p-5" @submit.prevent="login" >
                    ...
                    <p class="text-sm font-normal text-gray-600 mb-8">Welcome Back to Notes</p>
...
<style>
    .login_img_section {
        background: linear-gradient(rgba(2, 2, 2, .7), rgba(0, 0, 0, .7)), url('/img/background_login.jpg') center center;
    }
</style>
<script>
import api from '../api'
...
export default {
    ...
    methods: {
        async login() {
            ...
            try {
                const response = await api.post('/login', userData); // Ganti '/login' dengan endpoint login Anda
                console.log(response);
                const token = response.data.authorization.token; // Sesuaikan dengan respons Anda
                console.log(token);
                ...
                // Tangani kesalahan atau pesan kesalahan login di sini
